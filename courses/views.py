...
def course_list(request, category_slug=None, tag_slug=None):
    ...
    if category_slug != None:
        ...
        if current_user.is_authenticated:
            ...
            courses = Course.objects.all().order_by('-date')
            #kaydetmedigi kursları gostermek hepsini gostermeye grek yok
...
def course_detail(request, category_slug, course_id):
    current_user = request.user
    course = Course.objects.get(category__slug=category_slug, id = course_id)  ##__ category modelinden slug icin
    courses = Course.objects.all().order_by('-date')
    categories = categories = Category.objects.all()
    tags = Tag.objects.all()
    if current_user.is_authenticated:
...
""" def category_list(request, category_slug):
    courses = Course.objects.all().filter(category__slug=category_slug)  #ALTERNATIVE
    categories = Category.objects.all()
    tags= Tag.objects.all()
...
""" def tag_list(request, tag_slug):
    courses = Course.objects.all().filter(tags__slug=tag_slug) #ALTERNATIVE
    categories = Category.objects.all()
    tags= Tag.objects.all()
